  ...
  <script src="https://cdnjs.cloudflare.com/ajax/libs/Chart.js/4.4.0/chart.umd.min.js"></script>
    ...
    <div class="card card-wide" id="conteudo">
      <div class="loader"><div class="spinner"></div>
        ...
        <p style="color:#64748b">Buscando dados de ${fmtBR(inicioMes)} a ${fmtBR(fimMes)}</p>
      </div>
    </div>
    <script>
...
app.get("/api/contas-pagar", async (req, res) => {
  try {
    ...
    // Busca contas paginando e para quando sair do período
    let pagina = 1, contas = [];
    let sair = false;
    while (!sair) {
      ...
      if (pagina === 1) console.log("DEBUG CONTA:", JSON.stringify(itens[0], null, 2).substring(0, 500));

      for (const item of itens) {
        const dataItem = item.dataVencimento || item.data_vencimento || item.competencia || item.data || "";
        // Para quando data sair do período
        if (dataItem && dataItem > fim) continue;
        if (dataItem && dataItem < inicio) { sair = true; break; }
        contas.push(item);
      }

      console.log(`Contas a pagar p${pagina}: ${itens.length} | no período: ${contas.length}`);
      if (itens.length < 100) break;
      pagina++;
    }
