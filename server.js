...
app.get("/api/contas-pagar", async (req, res) => {
  try {
    ...
    // Tenta buscar com filtro de período por vencimento
    let pagina = 1, contas = [];
    while (true) {
      ...
      contas = contas.concat(itens);
      console.log(`Contas p${pagina}: ${itens.length} | total: ${contas.length}`);
      if (itens.length < 100) break;
      pagina++;
    }

    // Filtra localmente por dataVencimento no período (caso API ignore filtro)
    contas = contas.filter(c => {
      const dv = c.dataVencimento || c.data || "";
      return !dv || (dv >= inicio && dv <= fim);
    });
    console.log(`Total após filtro: ${contas.length}`);
    ...
    for (const c of contas) {
      ...
      const pago = sit === "pago" || sit === "baixado" || (c.saldo !== undefined && parseFloat(c.saldo) === 0 && parseFloat(c.valor) > 0);
      const dataVenc = c.dataVencimento || c.data || "";
      const vencido = !pago && dataVenc && dataVenc < hoje;
      const valor = parseFloat(c.valor || 0);
      const cat = c.categoria?.descricao || c.categoria?.nome || "Sem categoria";

      const item = {
        id: c.id,
        descricao: c.historico || c.descricao || "",
        categoria: cat,
        valor,
