...
async function sincronizarVendas(token) {
  ...
  try {
    ...
    while (true) {
      let data;
      try {
        data = await tinyGet(token, "pedidos", { pagina, limite: 100 });
      } catch(e) {
      ...
      for (const pedido of pedidos) {
        ...
        // Para quando data for anterior ao período
        if (dataCriacao && dataCriacao < dataInicioStr) {
          saiuDoPeriodo = true;
          break;
        }
        // Ignora cancelados (situacao=5 é cancelado no Tiny)
      ...
      if (saiuDoPeriodo || pedidos.length < 100) break;
      pagina++;
    }
