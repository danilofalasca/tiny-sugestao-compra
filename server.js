  ...
  try {
    ...
    while (true) {
      ...
      for (const pedido of pedidos) {
        // Ignora cancelados
        const status = (pedido.situacao || pedido.status || "").toLowerCase();
        if (status.includes("cancelad")) continue;

        // Extrai itens do pedido
        const itens = pedido.itens || pedido.items || pedido.produtos || [];
        for (const item of itens) {
          const sku = item.codigo || item.sku || item.produto?.codigo || "";
          const nome = item.descricao || item.nome || item.produto?.nome || "";
