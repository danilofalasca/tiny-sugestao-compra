    ...
    <div class="card">
      ...
      ${auth
        ? `<div class="ok">✅ Autenticado com sucesso!</div>
           ...
           <a href="/contas-pagar" class="btn btn-blue">💰 Contas a Pagar</a>
...
async function sincronizarVendas(token) {
  ...
  try {
    ...
    const dataInicioStr = fmt(inicio);

    let pagina = 1;
    let totalPedidos = 0;
    let totalItensComSku = 0;
    const vendasPorSku = {};

    console.log(`Iniciando sync de vendas a partir de ${dataInicioStr}...`);

    // PASSO 1: Busca IDs dos pedidos no período, parando quando sair da janela
    const idsPedidos = [];
    while (true) {
      let data;
      try {
        data = await tinyGet(token, "pedidos", { numeroPagina: pagina, limite: 100 });
      } catch(e) {
      ...
      for (const pedido of pedidos) {
        const dataCriacao = pedido.dataCriacao || pedido.data || pedido.dataPedido || "";
        if (dataCriacao && dataCriacao < dataInicioStr) continue;
        const sit = pedido.situacao;
        if (sit === 5 || sit === "5" || String(sit).toLowerCase().includes("cancelad")) continue;
        idsPedidos.push(pedido.id);
      }

      console.log(`Página ${pagina}: ${pedidos.length} pedidos | IDs coletados: ${idsPedidos.length}`);

      if (pedidos.length < 100) break;
      pagina++;
    }

    console.log(`Total de pedidos no período: ${idsPedidos.length}. Buscando itens...`);

    // PASSO 2: Busca itens de cada pedido
    for (let i = 0; i < idsPedidos.length; i++) {
      const pedidoId = idsPedidos[i];
      try {
        const det = await tinyGet(token, `pedidos/${pedidoId}`);
        const pedido = det.pedido || det.data || det;
        if (i === 0) console.log("DEBUG ITEM:", JSON.stringify(det, null, 2).substring(0, 2000));
        ...
        if (i % 50 === 0) {
          console.log(`Itens: ${i+1}/${idsPedidos.length} pedidos | SKUs: ${Object.keys(vendasPorSku).length}`);
          // Atualiza progresso no banco
          await pool.query(
            "UPDATE sync_log SET total_pedidos=$1, total_itens=$2 WHERE id=$3",
            [totalPedidos, Object.keys(vendasPorSku).length, logId]
          );
        }
      } catch(e) {
        console.error(`Erro pedido ${pedidoId}:`, e.message);
      }
    }
...
app.get("/contas-pagar", async (req, res) => {
  ...
  res.send(`<!DOCTYPE html><html lang="pt-BR"><head><meta charset="UTF-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/><title>Contas a Pagar</title>${CSS}
  <style>
    ...
    .metric-label{font-size:12px;color:#64748b;margin-bottom:4px;font-weight:500}
    .metric-value{font-size:22px;font-weight:700;color:#1e293b}
    .metric-value.vermelho{color:#dc2626}
    .metric-value.laranja{color:#ea580c}
    .metric-value.verde{color:#16a34a}
    .metric-value.azul{color:#2563eb}
    .cat-bar{margin-bottom:10px}
    .cat-header{display:flex;justify-content:space-between;font-size:13px;margin-bottom:4px}
    .cat-nome{font-weight:600;color:#1e293b}
    .cat-valor{color:#64748b}
    .bar-bg{background:#f1f5f9;border-radius:99px;height:8px;overflow:hidden}
    .bar-fill{height:8px;border-radius:99px;background:linear-gradient(90deg,#2563eb,#7c3aed);transition:width .6s}
    .lista-conta{display:flex;justify-content:space-between;align-items:center;padding:10px 0;border-bottom:1px solid #f1f5f9;font-size:13px}
    .tab-btns{display:flex;gap:8px;margin-bottom:16px;flex-wrap:wrap}
    .tab-btn{padding:6px 16px;border-radius:20px;border:1.5px solid #e2e8f0;background:#fff;cursor:pointer;font-size:13px;font-weight:500;color:#64748b}
    .tab-btn.ativo{background:#2563eb;color:#fff;border-color:#2563eb}
  ...
  <script src="https://cdnjs.cloudflare.com/ajax/libs/Chart.js/4.4.0/chart.umd.min.js"></script>
    ...
    <div class="card card-wide" id="conteudo">
      <div class="loader"><div class="spinner"></div>
        <p style="font-weight:600;font-size:16px;margin-bottom:8px;color:#1e293b">Carregando contas a pagar...</p>
        <p style="color:#64748b">Buscando contas de ${fmtBR(inicioMes)} a ${fmtBR(fimMes)}</p>
      </div>
    </div>
    <script>
      fetch('/api/contas-pagar?inicio=${fmt(inicioMes)}&fim=${fmt(fimMes)}', { credentials: 'include' })
        .then(r => r.json())
        .then(data => {
        ...
        .catch(e => {
          document.getElementById('conteudo').innerHTML =
            '<div class="row"><a href="/" class="voltar">←</a><h2>Erro</h2></div>' +
            '<div class="erro">⚠️ ' + e.message + '</div>' +
      ...
      function renderDashboard(d) {
        ...
        // Header
        html += '<div class="row"><a href="/" class="voltar">←</a><h2>💰 Contas a Pagar</h2></div>';
        html += '<p class="sub" style="margin-bottom:20px">Mês atual · ' + d.totalContas + ' lançamentos · atualizado agora</p>';

        // Cards de métricas
        html += '<div class="cards-grid">';
        html += '<div class="metric"><div class="metric-label">Total do Mês</div><div class="metric-value azul">' + fmt(d.totalMes) + '</div></div>';
        html += '<div class="metric"><div class="metric-label">Já Pago</div><div class="metric-value verde">' + fmt(d.totalPago) + '</div></div>';
        html += '<div class="metric"><div class="metric-label">A Pagar</div><div class="metric-value azul">' + fmt(d.totalAberto) + '</div></div>';
        html += '<div class="metric"><div class="metric-label">Vencido</div><div class="metric-value vermelho">' + fmt(d.totalVencido) + '</div></div>';
        html += '</div>';

        // Abas
        html += '<div class="tab-btns">';
        html += '<button class="tab-btn ativo" onclick="showTab(this,'categorias')">📊 Por Categoria</button>';
        html += '<button class="tab-btn" onclick="showTab(this,'vencidas')">⚠️ Vencidas (' + d.contasVencidas.length + ')</button>';
        html += '<button class="tab-btn" onclick="showTab(this,'avencer')">📅 A Vencer (' + d.contasAvencer.length + ')</button>';
        html += '<button class="tab-btn" onclick="showTab(this,'pagas')">✅ Pagas (' + d.contasPagas.length + ')</button>';
        html += '</div>';

        // Tab: Categorias
        html += '<div id="tab-categorias" class="section">';
        html += '<canvas id="chartCat"></canvas>';
        html += '<div style="margin-top:20px">';
        const maxCat = Math.max(...d.categorias.map(c => c.total));
        d.categorias.forEach(cat => {
          const pct = maxCat > 0 ? (cat.total / maxCat * 100).toFixed(1) : 0;
          html += '<div class="cat-bar">';
          html += '<div class="cat-header"><span class="cat-nome">' + cat.nome + '</span><span class="cat-valor">' + fmt(cat.total) + ' <small style="color:#94a3b8">(' + cat.qtd + ' contas)</small></span></div>';
        ...
        // Tab: Vencidas
        html += '<div id="tab-vencidas" class="section" style="display:none">';
        if (d.contasVencidas.length === 0) {
          html += '<div class="ok" style="text-align:center;padding:24px">✅ Nenhuma conta vencida!</div>';
        } else {
          d.contasVencidas.forEach(c => {
            html += '<div class="lista-conta">';
            html += '<div><div style="font-weight:600;color:#dc2626">' + (c.descricao||c.historico||'Sem descrição') + '</div>';
            html += '<div style="color:#94a3b8;font-size:12px">' + (c.categoria||'Sem categoria') + ' · Venceu ' + c.dataVencimento + '</div></div>';
            html += '<div style="font-weight:700;color:#dc2626;white-space:nowrap">' + fmt(c.valor) + '</div>';
            html += '</div>';
          });
        }
        html += '</div>';
        ...
        // Tab: A Vencer
        html += '<div id="tab-avencer" class="section" style="display:none">';
        d.contasAvencer.forEach(c => {
          html += '<div class="lista-conta">';
          html += '<div><div style="font-weight:600">' + (c.descricao||c.historico||'Sem descrição') + '</div>';
          html += '<div style="color:#94a3b8;font-size:12px">' + (c.categoria||'Sem categoria') + ' · Vence ' + c.dataVencimento + '</div></div>';
          html += '<div style="font-weight:700;color:#2563eb;white-space:nowrap">' + fmt(c.valor) + '</div>';
          html += '</div>';
        });
        html += '</div>';

        // Tab: Pagas
        html += '<div id="tab-pagas" class="section" style="display:none">';
        d.contasPagas.forEach(c => {
          html += '<div class="lista-conta">';
          html += '<div><div style="font-weight:600;color:#64748b">' + (c.descricao||c.historico||'Sem descrição') + '</div>';
          html += '<div style="color:#94a3b8;font-size:12px">' + (c.categoria||'Sem categoria') + ' · Pago em ' + (c.dataPagamento||c.dataVencimento) + '</div></div>';
          html += '<div style="font-weight:700;color:#16a34a;white-space:nowrap">' + fmt(c.valor) + '</div>';
          html += '</div>';
        });
        html += '</div>';

        html += '<div class="rodape"><a href="/" class="btn btn-gray btn-sm">← Início</a></div>';
        document.getElementById('conteudo').innerHTML = html;
        ...
        new Chart(ctx, {
          ...
          data: {
            ...
            datasets: [{
              ...
              backgroundColor: d.categorias.map((_, i) => ['#2563eb','#7c3aed','#ea580c','#16a34a','#dc2626','#0891b2','#d97706','#9333ea'][i % 8]),
              borderRadius: 6,
            }]
          },
          options: {
...
app.get("/api/contas-pagar", async (req, res) => {
  try {
    ...
    // Busca todas as contas do mês (pagas + em aberto + vencidas)
    // Para quando data de vencimento sair do mês atual
    let pagina = 1, contas = [], parar = false;
    while (!parar) {
      const data = await tinyGet(token, "contas-pagar", { pagina, limite: 100 });
      const itens = data.itens || data.data || [];
      if (itens.length === 0) break;

      if (pagina === 1) console.log("DEBUG CONTA:", JSON.stringify(itens[0], null, 2).substring(0, 600));

      let temNoMes = false;
      for (const item of itens) {
        const dataVenc = item.dataVencimento || item.data_vencimento || item.data || "";
        if (!dataVenc) { contas.push(item); continue; }
      ...
      // Para quando nenhuma conta da página está no mês E a data mais recente é anterior ao início
      const datas = itens.map(i => i.dataVencimento || i.data || "").filter(Boolean);
      const maisRecente = datas.length ? datas.reduce((a,b) => a > b ? a : b) : "";
      const maisAntiga = datas.length ? datas.reduce((a,b) => a < b ? a : b) : "";
      console.log(`Contas p${pagina}: ${itens.length} | no mês: ${contas.length} | datas: ${maisAntiga} ~ ${maisRecente}`);

      if (maisRecente && maisRecente < inicio) { console.log("Saiu do período, parando."); break; }
      if (itens.length < 100) break;
      pagina++;
    }

    // Classifica cada conta
    const contasVencidas = [], contasAvencer = [], contasPagas = [];
    const categoriaMap = {};

    for (const c of contas) {
      const situacao = c.situacao ?? c.status ?? 0;
      const pago = situacao === 2 || situacao === "2" || String(situacao).toLowerCase().includes("pag") || String(situacao).toLowerCase().includes("baixad");
      const venc = c.dataVencimento || c.data_vencimento || "";
      const vencido = !pago && venc && venc < hoje;
      const valor = parseFloat(c.valor || c.valorOriginal || 0);
      const cat = c.categoria?.descricao || c.categoria?.nome || c.historico || "Sem categoria";

      const item = {
        id: c.id,
        descricao: c.historico || c.descricao || c.complemento || "",
        categoria: cat,
        valor,
        dataVencimento: venc,
        dataPagamento: c.dataPagamento || c.data_pagamento || "",
        situacao,
      };

      if (pago) contasPagas.push(item);
      else if (vencido) contasVencidas.push(item);
      else contasAvencer.push(item);

      // Agrupa por categoria
      if (!categoriaMap[cat]) categoriaMap[cat] = { nome: cat, total: 0, qtd: 0 };
      categoriaMap[cat].total += valor;
      categoriaMap[cat].qtd++;
    }

    const categorias = Object.values(categoriaMap).sort((a, b) => b.total - a.total);
    const totalMes = contas.reduce((s, c) => s + parseFloat(c.valor || 0), 0);
    const totalPago = contasPagas.reduce((s, c) => s + c.valor, 0);
    const totalVencido = contasVencidas.reduce((s, c) => s + c.valor, 0);
    const totalAberto = contasAvencer.reduce((s, c) => s + c.valor, 0);

    // Ordena vencidas por data (mais antigas primeiro)
    contasVencidas.sort((a, b) => a.dataVencimento.localeCompare(b.dataVencimento));
    contasAvencer.sort((a, b) => a.dataVencimento.localeCompare(b.dataVencimento));

    res.json({
      ok: true,
      periodo: { inicio, fim },
      totalContas: contas.length,
      totalMes, totalPago, totalVencido, totalAberto,
      categorias,
      contasVencidas,
      contasAvencer,
      contasPagas,
    ...
    console.error("Erro contas a pagar:", e.message);
